...
for process in processes:
        f = ROOT.TFile('2dhistos_scaled/{}_JMEs_CR.root'.format(process),'recreate')
        f.cd()
        JMEs = ["JES_up", "JES_down", "JER_up","JER_down","JMS_up","JMS_down","JMR_up","JMR_down"]
        for JME in JMEs:
                passname = 'PmjjvsPmY_CR_pass_{}'.format(JME)
                loosename = 'LmjjvsLmY_CR_loose_{}'.format(JME)
                failname = 'FmjjvsFmY_CR_fail_{}'.format(JME)        
